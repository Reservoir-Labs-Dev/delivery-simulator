...
public sealed class DeliveryConsumer : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _connection = await OpenWithRetryAsync(factory, "delivery-service-consumer", stoppingToken);
        ...
        _logger.LogInformation(
            "DeliveryConsumer started on queue '{Queue}' (routingKey '{RoutingKey}', prefetch={Prefetch})",
            _consumer.QueueName, _consumer.ConsumesRoutingKey, _consumer.PrefetchCount);

        stoppingToken.Register(() => _logger.LogInformation("DeliveryConsumer stopping"));
    }
    ...
    private async Task OnMessageReceivedAsync(object? sender, BasicDeliverEventArgs ea)
    {
        ...
        try
        {
            ...
            var handler = scope.ServiceProvider.GetRequiredService<OrderReadyHandler>();
    ...
    private void DeclareTopology(IModel channel)
    {
        ...
        DeclareRetryQueue(channel, "delivery.retry.1", 1_000);
        DeclareRetryQueue(channel, "delivery.retry.2", 2_000);
        DeclareRetryQueue(channel, "delivery.retry.3", 4_000);
